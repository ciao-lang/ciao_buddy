...
'$builder_hook'(desc_name('Ciao-Buddy')).
...
'$builder_hook'(build_libraries) :-
	build_libs(ciao_buddy, 'src').

'$builder_hook'(install) :- bundleitem_do(only_global_ins(~ciao_buddy_desc), ciao_buddy, install).

'$builder_hook'(uninstall) :- bundleitem_do(only_global_ins(~ciao_buddy_desc), ciao_buddy, uninstall).
...
'$builder_hook'(runtests) :- !,
	working_directory(ThisDir, ~fsR(bundle_src(ciao_buddy))),
	invoke_ciaosh_batch([
...
aux_sh := ~fsR(bundle_src(ciao_buddy)/'Manifest'/'hooks.sh').
